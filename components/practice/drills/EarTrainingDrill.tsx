...
export function EarTrainingDrill({ exercise, attempts, onAttempt, onFinish }: DrillProps<EarTrainingExercise>) {
  const { playNote } = useGuitarSound()
  const [questionIndex, setQuestionIndex] = useState(0)
  const promptedAt = useRef(0)
  const answer = useMemo(() => CHOICES[questionIndex % CHOICES.length], [questionIndex])
  useEffect(() => {
    promptedAt.current = performance.now()
  }, [])
  const play = async () => {
    await playNote(261.63)
    window.setTimeout(() => void playNote(261.63 * 2 ** (answer.semitones / 12)), 650)
  }
  const choose = useCallback((choice: typeof CHOICES[number]) => {
    onAttempt(createAttempt({
      ...
      target: { skill: "earTraining", key: `interval:${answer.interval}`, interval: answer.interval },
    }))
  ...
  }, [answer.interval, attempts.length, exercise.questionCount, onAttempt, onFinish])
  return (
    <section className="mx-auto max-w-2xl space-y-7 text-center">
      <p className="font-mono text-xs tracking-[0.18em] text-muted-foreground uppercase">Ear training · {attempts.length + 1}/{exercise.questionCount}</p>
      <h2 className="text-3xl font-semibold">What interval do you hear?</h2>
      <Button onClick={() => void play()}>Play interval</Button>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {CHOICES.map((choice) => <Button key={choice.interval} variant="outline" onClick={() => choose(choice)}>{choice.label}</Button>)}
      </div>
    </section>
  )
}
